...
public class FindsDepartsChildrenUtil {
     ...
     */
    public static List<SysDepartTreeModel> wrapTreeDataToTreeList(List<SysDepartDO> recordList) {
        // 在该方法每请求一次,都要对全局list集合进行一次清理
        //idList.clear();
    	List<DepartIdModel> idList = Lists.newArrayList();
        List<SysDepartTreeModel> records = new ArrayList<>();
        for (int i = 0; i < recordList.size(); i++) {
            SysDepartDO depart = recordList.get(i);
     ...
     * @return
     */
    public static List<DepartIdModel> wrapTreeDataToDepartIdTreeList(List<SysDepartDO> recordList) {
        // 在该方法每请求一次,都要对全局list集合进行一次清理
        //idList.clear();
        List<DepartIdModel> idList = Lists.newArrayList();
        List<SysDepartTreeModel> records = new ArrayList<>();
